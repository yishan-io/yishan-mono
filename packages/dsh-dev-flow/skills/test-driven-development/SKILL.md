...
When using DSH subagents:

- an implementation subagent should follow TDD whenever the task involves behavior changes that are testable
- a review subagent should treat missing test-first discipline as a real quality concern when the task clearly called for it
- a review subagent can treat weak or suspicious test coverage as a merge risk

When calling `subagent` for implementation, include the expected verification commands so the red and green steps are concrete.
