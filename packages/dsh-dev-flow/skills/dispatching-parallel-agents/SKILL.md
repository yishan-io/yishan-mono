...
In those cases, execute the plan sequentially in the current session or use `subagent-driven-development` instead.
...
Use the native `subagent` tool with a capability-specific brief, for example:

- a research subagent for codebase search or investigation
- an implementation subagent for independent implementation tasks
- a read-only review subagent for separate review scopes
...
- execute a plan sequentially in the current session when one coherent context is needed
- `subagent-driven-development` = execute one plan with isolated sequential task handoffs
- `dispatching-parallel-agents` = use concurrent `subagent` calls for multiple independent scopes
