...
- use this skill before calling `subagent` for a speculative bug fix
- if an implementation subagent or the controller cannot explain the failure clearly, switch into debugging mode first
- use a fresh, read-only review subagent to catch fixes that patched symptoms but missed the real issue
