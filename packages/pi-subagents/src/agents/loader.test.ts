...
describe("loadAgentDefinitions", () => {
  it("applies user runtime overrides after user definitions but before project definitions", () => {
    const tempDir = createTempDir();
    ...
    const overridesPath = join(tempDir, "agent.overrides.json");
...
General prompt`,
    ...
    writeFileSync(
      overridesPath,
      JSON.stringify({
        Explore: { model: "overridden-model", thinking: "high" },
        General: { model: "general-model", thinking: "minimal" },
        Unknown: { tools: ["deploy"] },
      }),
      "utf8",
    );
    ...
    const result = loadAgentDefinitions({
      ...
      userAgentOverridesPath: overridesPath,
    });
  ...
  it("returns diagnostics for invalid known runtime overrides", () => {
    const tempDir = createTempDir();
    const builtinDir = join(tempDir, "builtin-agents");
    const overridesPath = join(tempDir, "agent.overrides.json");
    writeAgentFile(
...
Builtin prompt`,
    ...
    writeFileSync(overridesPath, JSON.stringify({ Explore: { thinking: "invalid" } }), "utf8");

    const result = loadAgentDefinitions({
      cwd: tempDir,
      builtinAgentsDir: builtinDir,
      userAgentsDir: join(tempDir, "user-agents"),
      projectAgentsDir: null,
      userAgentOverridesPath: overridesPath,
    });

    expect(result.agents).toContainEqual(expect.objectContaining({ name: "Explore", thinking: undefined }));
    expect(result.diagnostics).toEqual([
      {
