...
describe("SessionRuntime subscribe", () => {
  it("returns an empty live baseline before lazy persistence materializes", async () => {
    ...
    await expect(harness.runtime.subscribe({ cwd: CWD, sessionId: "one", afterSeq: -1 })).resolves.toMatchObject({
      events: [{ seq: 0, type: "yishan/session-bound.v1", data: BINDING }],
      asOfSeq: 0,
      durableThroughSeq: 0,
      headSeq: 0,
  ...
  it("rejects cursor zero for an empty durable session instead of echoing it", async () => {
    const harness = createHarness();
    await harness.runtime.start({ cwd: CWD, sessionId: "one", binding: BINDING });
    harness.readFrom.mockResolvedValueOnce({ meta: { id: "one", cwd: CWD }, events: [] });

    await expect(harness.runtime.subscribe({ cwd: CWD, sessionId: "one", afterSeq: 0 })).rejects.toMatchObject({
  ...
  it("slices the physical durable snapshot after the requested cursor", async () => {
    ...
    await expect(harness.runtime.subscribe({ cwd: CWD, sessionId: "one", afterSeq: 0 })).resolves.toMatchObject({
  ...
  it("rejects a persisted cursor above the physical durable head", async () => {
    ...
    await expect(harness.runtime.subscribe({ cwd: CWD, sessionId: "one", afterSeq: 4 })).rejects.toMatchObject({
      code: "YISHAN_SESSION_REPLAY_RESET_REQUIRED",
    });
  });
  ...
  it("reports physical durable cursors while live events append during and after the read", async () => {
    ...
    const subscribing = harness.runtime.subscribe({ cwd: CWD, sessionId: "one", afterSeq: -1 });
  ...
  it("rejects an owned live subscription whose durable baseline misses the pre-flush target", async () => {
    ...
    await expect(harness.runtime.subscribe({ cwd: CWD, sessionId: "one", afterSeq: -1 })).rejects.toMatchObject({
