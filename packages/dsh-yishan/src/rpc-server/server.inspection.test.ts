...
type Harness = { ctx: Context; input: PassThrough; frames: Record<string, unknown>[]; server: RpcServer };
...
describe("RpcServer session inspection", () => {
  ...
  it("maps durable reads and resume/dispose results through the RPC server", async () => {
    ...
    try {
      ...
      harness.input.write(
        `${JSON.stringify({ jsonrpc: "2.0", id: 15, method: YISHAN_METHODS.resume, params: { cwd: "/workspace", sessionId: "session-1" } })}\n`,
      );
      ...
      expect(resume).toHaveBeenCalledWith({ cwd: "/workspace", sessionId: "session-1" });
