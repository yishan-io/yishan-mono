...
import { listProviders, validateProviderSelection } from "../provider/providers";
import {
...
describe("Yishan production runtime without an LLM adapter", () => {
  it("accepts only the exact test-only replay environment value", () => {
    expect(isDshTestReplayEnabled({})).toBe(false);
    expect(isDshTestReplayEnabled({ [YISHAN_DSH_TEST_REPLAY_ENVIRONMENT_VARIABLE]: "true" })).toBe(false);
    expect(isDshTestReplayEnabled({ [YISHAN_DSH_TEST_REPLAY_ENVIRONMENT_VARIABLE]: "1 " })).toBe(false);
    expect(
      isDshTestReplayEnabled({
  ...
  it("includes the replay route in catalog validation only with the exact test flag", async () => {
    ...
    try {
      ...
      await expect(
        validateProviderSelection(runtime.context.llm, {
          provider: YISHAN_DSH_TEST_REPLAY_PROVIDER,
