...
import { refreshWorkspacePullRequest } from "@/features/workspaces/workspaces.api";
...
export function useWorkspaceCurrentPullRequestQuery(
  ...
  return useQuery({
    queryKey: queryKeys.workspaceCurrentPullRequest(organizationId, projectId, workspaceId),
    queryFn: async () => {
      return refreshWorkspacePullRequest(
