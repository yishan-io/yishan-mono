...
export function getGoogleOAuthScheme(): string {
  ...
  return required("EXPO_PUBLIC_GOOGLE_OAUTH_IOS_SCHEME", "");
}
...
export function getGoogleOAuthRedirectUri(): string {
  ...
  throw new Error("Missing required environment variable: EXPO_PUBLIC_GOOGLE_OAUTH_IOS_SCHEME");
}

export function isGoogleOAuthRedirectUrl(url: string): boolean {
  const configuredRedirectUri = getGoogleOAuthRedirectUri();

  return isSameLinkTarget(url, configuredRedirectUri);
}
