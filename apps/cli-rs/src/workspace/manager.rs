...
impl WorkspaceManager {
    ...
    pub fn git_list_changes(
        &self,
        workspace_id: &str,
    ) -> Result<GitChangesBySection, DomainRpcError> {
        let root = self.ws_path(workspace_id)?;
    ...
    pub fn git_worktree_create(
        ...
        let root = self.ws_path(workspace_id)?;
        self.gits
            .create_worktree(&root, branch, wt_path, create, from)
    }
