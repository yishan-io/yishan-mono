...
export function DaemonCliInstallCard(props: DaemonCliInstallCardProps) {
  ...
  // Only allow uninstalling when the daemon created the managed symlink. If the
  // binary was installed independently (managedInstall === false), it must not be removed.
  const canUninstall = isInstalled && Boolean(status?.managedInstall);
