...
describe("AgentDefinitionDialogsEdit", () => {
  ...
  it("editing folds the selector values into the frontmatter only when they change", async () => {
    ...
    // Only the changed selector value is folded in — the untouched model stays
    // out of the frontmatter (the file remains authoritative for it).
    await waitFor(() =>
      expect(mocked.updateAgentDefinition).toHaveBeenCalledWith({
        name: "my-helper",
        content: "---\nname: my-helper\ndescription: My custom helper\nthinking: xhigh\n---\n# body\n",
  ...
  it("editing description and tools rebuilds the frontmatter from the structured fields", async () => {
    mocked.listAgentDefinitions.mockResolvedValue([USER]);
    mocked.getAgentDefinitionDetail.mockResolvedValue({
      ...USER,
      content: USER_CONTENT,
    ...
    await screen.findByLabelText("settings.customize.agents.dialogs.edit.descriptionLabel");
    fireEvent.change(screen.getByLabelText("settings.customize.agents.dialogs.edit.descriptionLabel"), {
      target: { value: "Renamed helper" },
    });
    ...
    fireEvent.change(screen.getByLabelText("settings.customize.agents.dialogs.edit.contentLabel"), {
  ...
  it("editing the body alone keeps the frontmatter verbatim", async () => {
    mocked.listAgentDefinitions.mockResolvedValue([USER]);
    mocked.getAgentDefinitionDetail.mockResolvedValue({
      ...USER,
      content: "---\nname: my-helper\ndescription: Helper\nread_only: false\n---\n# old body\n",
  ...
  it("editing an official agent shows the overwrite confirmation before saving", async () => {
    ...
    // The markdown editor shows only the prompt body — never the frontmatter.
    const officialTextarea = await screen.findByLabelText("settings.customize.agents.dialogs.edit.contentLabel");
    expect((officialTextarea as HTMLTextAreaElement).value).toBe("# shipped\n");
    fireEvent.click(screen.getByTestId("agent-detail-save"));
  ...
  it("editing a user agent saves without an overwrite confirmation", async () => {
    ...
    expect((userTextarea as HTMLTextAreaElement).value).toBe("# body\n");
    fireEvent.click(screen.getByTestId("agent-detail-save"));
