...
describe("LocalTaskTemplateControls", () => {
  ...
  it("saves a custom template through the daemon command and permits its deletion", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "localTask.templates.name" }), { target: { value: "Feature" } });
  ...
  it("disables template controls while task creation is pending", () => {
    ...
    expect(screen.getByRole("combobox", { name: "localTask.templates.select" }).getAttribute("aria-disabled")).toBe("true");
