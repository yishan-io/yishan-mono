...
type dshTaskKeyAllocator struct{}

func (dshTaskKeyAllocator) AllocateTaskKey(_ context.Context, _ nodelocaltask.KeyAllocationRequest) (string, error) {
	return "TASK-1", nil
}
...
func newDSHTaskTestService(t *testing.T) (*nodelocaltask.Service, dshTaskScope) {
	...
	service := nodelocaltask.NewService(nodelocaltask.Deps{
		Repository: sqlite.NewLocalTaskStore(database), WorkspaceStore: sqlite.NewStore(workspaceStore), KeyAllocator: dshTaskKeyAllocator{},
	})
