...
import (
	...
	"yishan/apps/cli/internal/agent/command"
)

// ── BuildRunCommand ───────────────────────────────────────────────────────────

func TestBuildRunCommand_OpenCode_NonInteractive(t *testing.T) {
	cmd, err := command.BuildRunCommand("opencode", "summarize this", "", false)
	if err != nil {
...
func TestBuildRunCommand_Claude_NonInteractive(t *testing.T) {
	cmd, err := command.BuildRunCommand("claude", "hello", "", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBuildRunCommand_Pi_NonInteractive_UsesPromptModelAndNoSessionFlags(t *testing.T) {
	cmd, err := command.BuildRunCommand("pi", "summarize this", "openai/gpt-5", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if cmd.Binary != "pi" {
...
func TestBuildRunCommand_Pi_Interactive_NoNoSessionFlag(t *testing.T) {
	cmd, err := command.BuildRunCommand("pi", "hello", "", true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if cmd.Binary != "pi" {
...
func TestBuildRunCommand_UnsupportedAgent(t *testing.T) {
	_, err := command.BuildRunCommand("unknown-agent-xyz", "prompt", "", false)
	if err == nil {
...
func TestBuildRunCommand_EmptyAgentKind_DefaultsToOpenCode(t *testing.T) {
	cmd, err := command.BuildRunCommand("", "prompt", "", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if cmd.Binary != "opencode" {
...
func TestBuildRunCommand_OpenCode_NonInteractive_HasOPENCODE_DB(t *testing.T) {
	cmd, err := command.BuildRunCommand("opencode", "summarize", "", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	found := false
...
func TestBuildRunCommand_OpenCode_Interactive_NoExtraEnv(t *testing.T) {
	cmd, err := command.BuildRunCommand("opencode", "hello", "", true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBuildRunCommand_Codex_NonInteractive_HasEphemeralFlag(t *testing.T) {
	cmd, err := command.BuildRunCommand("codex", "hello", "", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	found := false
...
func TestResolveCommandWithEnv_BinaryNotFound_ReturnsError(t *testing.T) {
	...
	_, err := command.ResolveCommandWithEnv("opencode", "prompt", "", false, env)
	if err == nil {
...
func TestResolveCommandWithEnv_BinaryFound_ReturnsAbsolutePath(t *testing.T) {
	...
	cmd, err := command.ResolveCommandWithEnv("opencode", "prompt", "", false, env)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestResolveCommandWithEnv_UnsupportedAgent_ReturnsError(t *testing.T) {
	...
	_, err := command.ResolveCommandWithEnv("unknown-xyz", "prompt", "", false, env)
	if err == nil {
		t.Fatal("expected error for unsupported agent kind")
	}
	if !strings.Contains(err.Error(), "unsupported agent kind") {
...
func TestResolveCommandWithEnv_ArgsPreserved(t *testing.T) {
	...
	env := []string{"PATH=" + binDir}
	cmd, err := command.ResolveCommandWithEnv("opencode", "my prompt", "gpt-4o", false, env)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
