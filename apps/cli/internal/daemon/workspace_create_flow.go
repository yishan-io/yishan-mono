...
type preparedWorkspaceCreate struct {
	...
	isRelayed        bool // true when this create was dispatched from another node via relay
...
func (h *JSONRPCHandler) prepareWorkspaceCreate(ctx context.Context, req workspaceCreateParams) (preparedWorkspaceCreate, error) {
	...
	if isDirectWorkspaceCreateRequest(normalized) {
		if normalized.NodeID == "" {
			normalized.NodeID = h.nodeID
		}
		return prepareDirectWorkspaceCreate(normalized, h.nodeID), nil
	}
...
func normalizeWorkspaceCreateParams(req workspaceCreateParams) workspaceCreateParams {
	...
	if req.Kind == "" {
		req.Kind = string(workspace.KindWorktree)
	}
	return req
}
...
func prepareDirectWorkspaceCreate(req workspaceCreateParams, sourceNodeID string) preparedWorkspaceCreate {
	...
	var registration *WorkspaceCreation
	if req.OrganizationID != "" && req.ProjectID != "" && req.NodeID != "" {
		registration = &WorkspaceCreation{
			ID:             req.ID,
			NodeID:         req.NodeID,
...
func (h *JSONRPCHandler) prepareWorktreeWorkspaceCreate(ctx context.Context, req workspaceCreateParams) (preparedWorkspaceCreate, error) {
	...
	registration := WorkspaceCreation{ID: req.ID, NodeID: nodeID, SourceNodeID: h.nodeID, OrganizationID: req.OrganizationID, ProjectID: req.ProjectID, Kind: string(workspace.KindWorktree), Branch: req.Branch, SourceBranch: req.SourceBranch}
	return preparedWorkspaceCreate{workspaceID: req.ID, organizationID: req.OrganizationID, projectID: req.ProjectID, startedEvent: buildWorkspaceCreateStartedEvent(req, nodeID, req.Branch), relayReplyNodeID: req.ReplyNodeID, isRelayed: req.ReplyNodeID != "", localCreate: &createReq, registration: &registration}, nil
}
...
func prepareRemoteWorkspaceCreate(req workspaceCreateParams, targetNodeID string, replyNodeID string) preparedWorkspaceCreate {
	...
	registration := WorkspaceCreation{ID: req.ID, NodeID: targetNodeID, SourceNodeID: replyNodeID, OrganizationID: req.OrganizationID, ProjectID: req.ProjectID, Kind: string(workspace.KindWorktree), Branch: branch, SourceBranch: req.SourceBranch}
...
func resolvePrimaryWorkspaceForNode(runtime *cliruntime.Runtime, organizationID string, projectID string, nodeID string) (api.Workspace, error) {
	...
	for _, item := range workspacesResponse.Workspaces {
		if item.Kind == string(workspace.KindPrimary) && item.NodeID == nodeID && strings.TrimSpace(item.LocalPath) != "" {
			return item, nil
		}
	}
