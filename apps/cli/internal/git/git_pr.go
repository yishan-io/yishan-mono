...
func (s *GitService) branchPullRequest(ctx context.Context, root string, branch string, refresh bool, includeDetails bool) (GitBranchPullRequestStatus, error) {
	...
	if branchName == "" {
		return GitBranchPullRequestStatus{}, NewError(ErrCodeInvalidParams, "branch is required")
	}
	...
	if err := json.Unmarshal([]byte(out), &prs); err != nil {
		return GitBranchPullRequestStatus{}, NewError(ErrCodeToolUnavailable, "failed to parse gh pr list output")
	}
