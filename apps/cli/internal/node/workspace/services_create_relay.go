...
func (s *Service) dispatchCreate(req workspaceCreateParams, started workspaceCreateStartedEvent) error {
...
func (s *Service) relayCreateProgress(prepared preparedWorkspaceCreate, event workspace.CreateProgressEvent) {
	...
	if err := s.sendSnapshotNotification(payload); err != nil {
		log.Warn().Err(err).Str("workspaceId", prepared.WorkspaceID).Msg("relay workspace create progress failed")
	}
}

func (s *Service) RelayCreateCompleted(prepared preparedWorkspaceCreate, completed map[string]any) {
	if strings.TrimSpace(prepared.RelayReplyNodeID) == "" {
		return
	}
...
func (s *Service) relayCreateFailed(prepared preparedWorkspaceCreate, failed workspaceCreateFailedEvent) {
	if strings.TrimSpace(prepared.RelayReplyNodeID) == "" {
		return
	}
...
func (s *Service) sendSnapshotNotification(payload relayWorkspaceCreateEnvelope) error {
...
func (s *Service) handleRelayedCreate(payload relayWorkspaceCreateEnvelope) {
	if payload.Request == nil || strings.TrimSpace(payload.TargetNodeID) != s.deps.NodeID {
		return
	}
	if err := s.app.ExecuteRelayed(s.serverContext(), workspaceCreateParams(*payload.Request)); err != nil {
		failed := workspaceCreateFailedEvent{WorkspaceID: payload.WorkspaceID, Message: err.Error()}
		s.relayCreateFailed(preparedWorkspaceCreate{WorkspaceID: payload.WorkspaceID, OrganizationID: payload.OrganizationID, ProjectID: payload.ProjectID, RelayReplyNodeID: strings.TrimSpace(payload.SourceNodeID)}, failed)
	}
}

func (s *Service) republishCreate(payload relayWorkspaceCreateEnvelope) {
