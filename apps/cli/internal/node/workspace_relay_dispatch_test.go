...
func TestSendRelayDispatchRequest_RejectedWhenTargetOffline(t *testing.T) {
	s := newTestHandler(t)
	wireRelayReader(t, s, map[string]any{"accepted": false, "reason": "target node offline"})

	err := s.relayClient.SendDispatchRequest(
...
func TestSendRelayDispatchRequest_AcceptedWhenTargetOnline(t *testing.T) {
	s := newTestHandler(t)
	wireRelayReader(t, s, map[string]any{"accepted": true})

	if err := s.relayClient.SendDispatchRequest(
