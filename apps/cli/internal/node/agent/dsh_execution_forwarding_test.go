...
func TestDSHExecution_ForwardsLifecycleAndResyncUnchanged(t *testing.T) {
	runtime := &executionDSH{subscribeInstanceID: "inc-1"}
	...
	updates <- dsh.SessionUpdate{Lifecycle: &dsh.SubagentLifecycle{
		Version: 1, ParentSessionID: "s", InstanceID: "inc-1", Revision: 7,
		Event: "started", RunID: "run-1", ChildSessionID: "child-1", Provider: "pi", Local: true,
	}}
	updates <- dsh.SessionUpdate{LifecycleResync: &dsh.LifecycleResync{ParentSessionID: "s", InstanceID: "inc-1", Revision: 7}}

	assertDSHForwardedUpdate(t, client, "inc-1", `{"lifecycle":{"version":1,"parentSessionId":"s","instanceId":"inc-1","revision":7,"event":"started","runId":"run-1","childSessionId":"child-1","provider":"pi","local":true}}`)
	assertDSHForwardedUpdate(t, client, "inc-1", `{"lifecycleResync":{"parentSessionId":"s","instanceId":"inc-1","revision":7}}`)
...
func TestDSHExecution_LifecycleForwardingIsolatesStaleRouteGeneration(t *testing.T) {
	...
	staleUpdates <- dsh.SessionUpdate{Lifecycle: &dsh.SubagentLifecycle{Version: 1, ParentSessionID: "s", InstanceID: "inc-1", Revision: 8, Event: "started", RunID: "stale", ChildSessionID: "child", Provider: "pi", Local: true}}
	close(staleUpdates)
	...
	runtime.mu.Unlock()
	updates <- dsh.SessionUpdate{LifecycleResync: &dsh.LifecycleResync{ParentSessionID: "s", InstanceID: "inc-1", Revision: 8}}
	assertDSHForwardedUpdate(t, newClient, "inc-1", `{"lifecycleResync":{"parentSessionId":"s","instanceId":"inc-1","revision":8}}`)
}

func assertDSHForwardedUpdate(t *testing.T, client interface {
	SetReadDeadline(time.Time) error
	ReadJSON(any) error
}, wantInstanceID, wantUpdate string) {
	t.Helper()
	...
	if notification.Params.Topic != dshEventTopic || notification.Params.Payload.SessionID != "s" || notification.Params.Payload.TabID != "tab" || notification.Params.Payload.WorkspaceID != "w" || notification.Params.Payload.InstanceID != wantInstanceID {
